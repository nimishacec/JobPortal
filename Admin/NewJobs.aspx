...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="row">
                                ...
                                <asp:Repeater ID="rptJobs" runat="server">
                                    <ItemTemplate>
                                        <div class="row job-row" data-jobid='<%# Eval("EmployeeID") %>'>
                                            ...
                                            ? "<div class='progress'><div class='badge badge-primary' role='progressbar' style='width: 80%;' aria-valuenow='50' aria-valuemin='0' aria-valuemax='50'>Completed</div></div>" 
     ...
     </div>      <div id="confirmationBox" class="modal">
    ...
    <script type="text/javascript">
        ...
        function updateJobStatus(jobID, status, element) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({ id: jobID, status: status }),
                dataType: "json",
                success: function (response) {
                    console.log("Response:", response);

                    // Check if the response was successful
                    if (response.d && JSON.parse(response.d).success) {
                        // Update the toggle button based on the new status
                        element.checked = (status === "APPROVED");

                        // Update the progress bar status
                        updateProgressBar(status);

                        alert("Operation successful: " + JSON.parse(response.d).message);
                    } else {
                ...
                error: function (xhr, status, error) {
                    ...
                    alert("Error: " + error);
                    // Reset toggle to its previous state in case of error
