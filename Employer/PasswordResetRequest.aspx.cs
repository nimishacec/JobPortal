        ...
        protected void btnRequestReset_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(email))
                {
                    ...
                    var pwdreset = _dataAccess.RequestResetPassword(token, email, expiryTime, "EMPLOYER");

                    string resetLink = $"{Request.Url.GetLeftPart(UriPartial.Authority)}/Employer/ResetPassword.aspx?token={token}";
